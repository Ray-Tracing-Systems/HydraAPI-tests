...
\begin{equation}\label{brdf_idealmirror}
...
\begin{equation}\label{brdf_idealmirror}
BRDF_{mirror} = \frac{1}{\max(cos(\theta_{out}), 1.0*10^{-6})}
\end{equation}

\textbf{!!!TODO: add test image for this test!!!}
...
\newline \textbf{!!!TODO: make this test inside cornell box!!!}
\newline \textbf{!!!TODO: the test must automaticly check impl. requirenment!!!}
\begin{figure}[htb]
	\centering
	\includegraphics[width=0.25\linewidth]{../../tests_images/test_123/w_ref.png}           
	\caption{\label{fig:test_123_image}
		Example of emissive material. }   
\end{figure}

\boldblue{(!) Implementation REQ. :} Emission color value \textbf{must} specify \textbf{brightness} value. It is easy to ckeck this requirenment. Directly visible emissive surface on rendered image (stored in linear HDR framebuffer) must have exactly same value that was set for material. For example if you set emission color to (0.5, 0.5, 0) and then check directly visiable surfaces (when primary rays immediately hit surface), you shoulg get exactly same yellow color (0.5, 0.5, 0) in the rendered image (in linear space, i.e. HDR image).

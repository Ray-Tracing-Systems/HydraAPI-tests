...
energyFix = \frac{1}{cos(\alpha)*cos(\theta)}
\end{equation}
...
\begin{figure}[htb]
	...
	\caption{\label{fig:phongletters}
		Surveys for equation \ref{eq:phongEnergyFix}.  }   
\end{figure}

Where $cos(\alpha)$ is the cosine used in Phong model and $cos(\theta)$ is geometry cosine (fig. \ref{fig:phongletters}).

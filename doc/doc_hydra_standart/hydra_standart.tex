...
At this specification level, the whole scene is defined as a single mesh or several meshes with transformed vertices in \underline{camera space}: camera located at (0,0,0) and looks at (0,0,+1). All positions of vertices must me specified \underline{in meters}. 
...
At this specification level, the whole scene is defined as a single mesh or several meshes with transformed vertices in \underline{world space}. The camera should be specified in the following way:
\begin{itemize}
...
The modelView matrix should be calculated from (position,lookAt,upVector) according to \href{https://www.khronos.org/registry/OpenGL-Refpages/gl2.1/xhtml/gluLookAt.xml}{gluLookAt} specification.

...
Where in practice $cos(\alpha) = dot(\omega_o, \omega_r)$ and $\omega_r$ is perfect specular reflection vector obtained from $\omega_i$: $\omega_r = reflect(\omega_i, n)$.
...
\reqirenmentextension{ggx}

The GGX isotropic BRDF implementation according to \cite{ggx}:
...
\begin{figure}[htb]
	\centering
	\includegraphics[width=0.25\linewidth]{../../3dsMaxTests/Reference/330_CornellBoxWithSphereCylinderTeapot.png}           

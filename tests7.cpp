...
bool test42_load_mesh_compressed()
{
  ...
  HRMaterialRef matY = hrMaterialCreate(L"yellow");
  ...
  hrMaterialOpen(matY, HR_WRITE_DISCARD);
  {
    xml_node matNode = hrMaterialParamNode(matY);
    xml_node diff    = matNode.append_child(L"diffuse");
    
    diff.append_attribute(L"brdf_type").set_value(L"lambert");
    diff.append_child(L"color").append_attribute(L"val").set_value(L"0.3 0.3 0.0");
  
    auto refl = matNode.append_child(L"reflectivity");
    refl.append_attribute(L"brdf_type").set_value(L"phong");
    refl.append_child(L"color").append_attribute(L"val").set_value(L"0.25 0.25 0.25");
  ...
  hrSceneOpen(scnRef, HR_WRITE_DISCARD);
  {
    ...
    //
    auto mtranslate1 = hlm::translate4x4(hlm::float3(-3, -4.0, 0));
    ...
    //// instance light (!!!)
    //
    auto mtranslate = hlm::translate4x4(hlm::float3(0, 3.85f, 0));

...
SimpleMesh CreateCube(float a_size)
{
  ...
  for (std::size_t i = 0; i < cube.vPos.size(); i++)
    cube.vPos[i] = cubeVertices[i] * a_size;

  for (std::size_t i = 0; i < cube.vNorm.size(); i++)
    cube.vNorm[i] = cubeNormals[i];

  for (std::size_t i = 0; i < cube.vTexCoord.size(); i++)
    cube.vTexCoord[i] = cubeTexCoords[i];

  for (std::size_t i = 0; i < cube.triIndices.size(); i++)
    cube.triIndices[i] = cubeIndices[i];

  for (std::size_t i = 0; i < cube.matIndices.size(); i++)
    cube.matIndices[i] = 0;
